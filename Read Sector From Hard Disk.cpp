// Support Me on Github : https://github.com/Monnalisa-ID
// Run as Adminstrator Agar code Bisa Jalan!
...
int ReadSectorsFromDisk(int disk, DISK_GEOMETRY dg, DWORD dMode, HANDLE hDrive){
	...
	while(!quit){
		...
		if(dMode == FINDINDEXOFFILE){
			...
			fflush(stdin);
			printf("\nContoh : Enter full path of file: c:\windows\system32\config\sam\n");
